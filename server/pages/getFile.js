...
module.exports = (req, conf, end, url) => {
  ...
  if (req.headers.cookie === undefined) {
    ...
    try {
      end(fs.readFileSync(`../tasks/${decodeURIComponent(url.split('/')[0])}/${decodeURIComponent(url.split('/')[1])}`).toString('utf-8'))
    } catch {
      end('Creepy Server Error: File or Task doesn\'t exists')
    }
  }
}
